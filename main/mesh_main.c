...
static uint8_t tx_buf[TX_SIZE] = { 7, };
static uint8_t rx_buf[RX_SIZE] = { 0, };
static bool is_running = true;
static bool is_mesh_connected = false;
static mesh_addr_t mesh_parent_addr, mesh_child_addr;
 ...
 *******************************************************/
mesh_addr_t fromLeaf;
mesh_data_t dataLeaf;


void esp_mesh_p2p_tx_main(void *arg)
{
    int i;
    esp_err_t errNodeSend;
    esp_err_t errRootSend;
    int send_count = 0;
...
//#endif /* MESH_P2P_TOS_OFF */
    ...
    while (is_running) {
        ...
        if (!esp_mesh_is_root()) 
        {
            ...
            int nodeLevel=esp_mesh_get_layer();
            if(nodeLevel==2){
                errNodeSend = esp_mesh_send(NULL, &data, MESH_DATA_P2P, NULL, 0);
                if(errNodeSend == ESP_OK)
                    ESP_LOGI(MESH_TAG, "Data sent from leaf");
                else ESP_LOGI(MESH_TAG, "Data send from leaf FAILED");
            }
        ...
        if (esp_mesh_is_root()) 
        {            
            errRootSend = esp_mesh_send(&mesh_child_addr, &data, MESH_DATA_P2P, NULL, 0);
            if(errRootSend == ESP_OK)
                ESP_LOGI(MESH_TAG, "Data sent from root");
            else
                ESP_LOGI(MESH_TAG, "Data send from root FAILED");
...
void esp_mesh_p2p_rx_main(void *arg)
{
    ...
    while (is_running) {
        ...
        if (esp_mesh_is_root()) 
        {
            ...
            errRootRcv = esp_mesh_recv(&fromLeaf, &dataLeaf, portMAX_DELAY, &flag, NULL, 0);
            //err = esp_mesh_recv(&from, &data, portMAX_DELAY, &flag, NULL, 0);     Took from esp car master
            if(errRootRcv == ESP_OK)
            {
                ESP_LOGI(MESH_TAG, "Data rcvd by root");
            }
            else
            {
                const char *arr =esp_err_to_name(errRootRcv);
                printf("%s\n",arr );
            }
        ...
        {
            ESP_LOGI(MESH_TAG, "Leaf node recieving nothing");
        }
            vTaskDelay(10 * 1000 / portTICK_RATE_MS);
...
esp_err_t esp_mesh_comm_p2p_start(void)
{
    ...
    if (!is_comm_p2p_started) {
        ...
        xTaskCreatePinnedToCore(esp_mesh_p2p_tx_main, "MPTX", 3072, NULL, 5, NULL, 0);  //Transmission in core 0
        xTaskCreatePinnedToCore(esp_mesh_p2p_rx_main, "MPRX", 3072, NULL, 5, NULL, 1);  //Transmission in core 1
    }
    return ESP_OK;
}
...
void mesh_event_handler(mesh_event_t event)
{
    ...
    case MESH_EVENT_CHILD_CONNECTED:
        ...
        memcpy(&mesh_child_addr.addr, event.info.child_connected.mac, 6);
